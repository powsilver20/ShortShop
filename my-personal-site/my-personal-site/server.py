...
@app.route('/mail/<id>/<email>')
def sendmail(id,email):
    if request.method == "POST":
        message = request.form['message']
        sendmessage = f"Hello {email} you have recived a message \n" \
                      f"sender: {id}\n" \
                      f"Content: {message}"

        server = smtplib.SMTP('smtp.gmail.com', 587)
        server.starttls()
        server.login("shortshop@mail.com" , "shortshop123")
        server.sendmail("shortshop@gmail.com", f"{email}",sendmessage)
        server.quit()
    return render_template('mail.html',id=id , email=email)


@app.route('/buyer/feed/<id>/<price>/<product>', methods=['GET', 'POST'])
def feedpage(id,price, product):
    if request.method == 'POST':
        if 'price' in request.form:
            ...
            if price == "£20":
                price = "20"
            return redirect(f'http://127.0.0.1:5000/buyer/feed/{id}/{price}/x')

        if 'email' in request.form:
            sqlconnection = sqlite3.Connection(currentlocation + "/database/users.db")
            cursor = sqlconnection.cursor()
            query5 = f"SELECT * From session"
            ...
            sqlconnection.close()
            rows4 = list(rows4)
            rows4 = rows4[len(rows4) - 1]


            return redirect(f"/mail/{id}/{rows4[3]}")
    ...
    cursor = sqlconnection.cursor()
    query4 = f"SELECT * From product WHERE category = '{select}' AND price = '{price}'"
    ...
    print("number of rows:",len(rows3))

    if (len(rows3)) < 1:
        rows = ["","","","","","",""]
        return render_template("error.html", rows=rows,price=price, select=select,)

    prodselect = random.randint(0, (len(rows3) - 1))
    print("product selected:",prodselect)
    ...
    sqlconnection = sqlite3.Connection(currentlocation + "/database/users.db")
    cursor = sqlconnection.cursor()
    query = "INSERT INTO session (productID, productname, description, sellerID, image, price, category) VALUES (?, ?, ?, ?, ?, ?, ?)"
    cursor.execute(query, (rows3[0], rows3[1], rows3[2], rows3[3], rows3[4], rows3[5], rows3[6]))
    sqlconnection.commit()
    sqlconnection.close()

    return render_template("feed.html",rows=rows3, id=id,price=price,)
...
def checklogin(type):
    ...
    if (len(rows)) == 1 and (type == "buyer"):
        return redirect(f"/buyer/feed/{UN}/5/x")
